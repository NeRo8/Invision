...
class ProfileNotificationsChat extends Component {
    constructor(props) {
        ...
        this.state = {
            data: [
                ...
                { date: "10:20 AM", type: 'in', message: "Hi Alex. How are you?", name: "Pavlo Laptev" },
    ...
    renderItem = ({ item }) => {
        ...
        if (item.type == "date") {
            return (
                <View>
                    <Text style={[globalStyles.gothamBook, styles.date]}>{item.when}</Text>
                </View>);
        }
                ...
                <View style={[styles.itemMe]}>
                    <View style={[styles.balloon]}>
                        <View style={styles.item}>
                            <Text style={[globalStyles.gothamBook, styles.messageMe]}>{item.message}</Text>
                        </View>
                    ...
                    <View style={[styles.itemIn]}>
                        <View style={[styles.balloon, { flexDirection: "row" }]}>
                            <Image
                                ...
                                style={{ width: 40, height: 40, borderRadius: 40 / 2, alignSelf: 'flex-end', marginRight: 15 }}
                            />
                            <View>
                                <View style={styles.itemInName}>
                                    <Text style={[globalStyles.gothamBold, styles.messageInTitle]}>{item.name}</Text>
                                </View>
...
export default withNavigation(ProfileNotificationsChat);